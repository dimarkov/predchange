...
class BayesianReversalLearner(object):
    ...
    def update_beliefs(self, t, observations, responses):
        ...
        ollNR = np.sum(np.log(mus[range(blocks), responses])*o, axis = -1)
        
        #likelihood of reversal state
        ollR = np.sum(np.log(mus[range(blocks), 1-responses])*o, axis = -1)
        ...
        ab_post[range(blocks), responses] += o*theta[:,None]
        ab_post[range(blocks), 1-responses] += o*(1-theta[:,None])
